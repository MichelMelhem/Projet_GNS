...
def insertion(txt: list, indicateur: str, contenu: list):
...
def BGP(datas: Dict[str, Router], r_name: str, igp_process_name: str):
    ...
    Renvoie :
        -le bloc de lignes pour BGP contenant les BGP Policies
    """
    ...
    if datas[r_name].is_border_router == True: #test si il faut mettre eBGP (routeur de bordure)
        for i in range(len(datas[r_name].neighbors_border_interfaces)):
            ...
            #ajout des tags aux routes entrantes 
            if datas[r_name].as_level==datas[datas[r_name].eBGP_neighbor[i]].as_level:
                BGP_bloc.append("  neighbor "+ datas[datas[r_name].eBGP_neighbor[i]].interfaces[datas[r_name].neighbors_border_interfaces[i]].ip.split('/')[0] +" route-map "+"PEER_POLICY in")
            elif datas[r_name].as_level<datas[datas[r_name].eBGP_neighbor[i]].as_level:
                BGP_bloc.append("  neighbor "+ datas[datas[r_name].eBGP_neighbor[i]].interfaces[datas[r_name].neighbors_border_interfaces[i]].ip.split('/')[0] +" route-map "+"PROVIDER_POLICY in")
            else:
                BGP_bloc.append("  neighbor "+ datas[datas[r_name].eBGP_neighbor[i]].interfaces[datas[r_name].neighbors_border_interfaces[i]].ip.split('/')[0] +" route-map "+"CUSTOMER_POLICY in")
